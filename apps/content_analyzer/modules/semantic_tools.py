...
def parse_faq_from_excel(uploaded_file, question_col: str, answer_col: str) -> List[Tuple[str, str]]:
    ...
    try:
        ...
        st.error(f"📁 Error de acceso al archivo: {e}")
        return []
    except (pd.errors.ParserError, ValueError) as e:
        st.error(f"⚠️ Error al parsear el archivo: {e}")
        return []
    except ImportError as e:
        st.error(f"📦 Falta librería (instala openpyxl o xlrd): {e}")
        return []
    ...
    if input_method == "📝 Entrada manual":
        ...
        if uploaded_faq_file is not None:
            try:
                ...
                st.error(f"⚠️ Error de formato en el archivo: {e}")
            except (FileNotFoundError, PermissionError) as e:
                st.error(f"📁 Error de acceso al archivo: {e}")
            except ImportError as e:
                st.error(f"📦 Falta librería para leer archivos: {e}")
            except Exception as e:
